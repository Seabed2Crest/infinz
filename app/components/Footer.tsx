...
function FooterLink({ href, children, onClick }: { href?: string, children: React.ReactNode, onClick?: () => void }) {
  ...
  return (
    <button
      ...
      className={`
        text-gray-600 hover:text-teal-600 transition-all duration-300 text-left
...
function SocialIcon({ icon: Icon, href, delay = 0 }: { icon: any, href: string, delay?: number }) {
  const [isHovered, setIsHovered] = useState(false);
  ...
  return (
    <a
      ...
      className={`
        w-10 h-10 rounded-full bg-gray-100 hover:bg-teal-100 
      ...
      onMouseLeave={() => setIsHovered(false)}
    >
      <Icon className={`w-5 h-5 text-gray-600 hover:text-teal-600 transition-colors duration-300`} />
    </a>
  );
}
  ...
  return (
    <footer className="bg-white py-16 border-t border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Main Footer Content */}
        <div ref={elementRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 mb-12">
          ...
          <FooterColumn title="" delay={0}>
            <div className="space-y-4">
              ...
              <p className="text-gray-600 text-sm leading-relaxed mb-6">
                ...
                Compare, choose, and get the best loan rates.
              </p>
        ...
        {/* Bottom Section */}
        <div className={`border-t border-gray-200 pt-8 transform transition-all duration-1000 ${
          isInView ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
        }`}
